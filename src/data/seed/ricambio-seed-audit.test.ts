...
const SERIES = ["hs", "hp", "hm", "xj", "xp", "sdw", "evo"] as const;
...
describe("Ricambio seed catalogue completeness", () => {
  for (const series of SERIES) {
    it(`${series.toUpperCase()} seed includes every Ricambio draft part`, () => {
      ...
      const uniqueDraftNumbers = [...new Set(draftNumbers)] as string[];
      const missing = uniqueDraftNumbers.filter((pn) => !seedParts.includes(pn));
