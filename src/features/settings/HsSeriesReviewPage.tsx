...
import {
  approveHsSeriesImport,
  getLatestHsDraft,
  saveImportDraft,
} from "@/services/hsSeriesImportService";
import { DAVEY_BASE } from "@/utils/brands";
import type { DraftPartRecord, HsSeriesImportDraft } from "@/types/catalogue-import";

export function HsSeriesReviewPage() {
  const router = useRouter();
  const [draft, setDraft] = useState<HsSeriesImportDraft | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      let existing = await getLatestHsDraft();
      if (!existing) {
        existing = getBundledHsDraft();
        await saveImportDraft(existing);
      }
      if (active) {
        setDraft(existing);
        setLoading(false);
  ...
  const fetchLive = async () => {
    ...
    try {
      const res = await fetch("/api/catalogue/fetch", {
        ...
        body: JSON.stringify({ includeDiagram: true, family: "hs" }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Fetch failed");
      await saveImportDraft(data.draft);
      setDraft(data.draft);
  ...
  const approveAllIllustrated = async () => {
    if (!draft) return;
    const next: HsSeriesImportDraft = {
      ...draft,
      parts: draft.parts.map((p) =>
  ...
  const approveImport = async () => {
    ...
    try {
      ...
      const history = await approveHsSeriesImport(draft);
      setMessage(
  ...
  if (loading && !draft) return <LoadingSpinner label="Loading HS Series review…" />;

  return (
    <div className="space-y-5 pb-8">
      <PageHeader title="HS Series Review" onBack={() => router.push(`${DAVEY_BASE}/settings`)} />
