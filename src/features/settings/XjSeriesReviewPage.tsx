...
import { getBundledXjDraft } from "@/lib/ricambio/parse-xj-series";
import {
  approveXjSeriesImport,
  getLatestXjDraft,
  saveXjImportDraft,
} from "@/services/xjSeriesImportService";
import { DAVEY_BASE } from "@/utils/brands";
import type { DraftPartRecord, XjSeriesImportDraft } from "@/types/catalogue-import";

export function XjSeriesReviewPage() {
  const router = useRouter();
  const [draft, setDraft] = useState<XjSeriesImportDraft | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      let existing = await getLatestXjDraft();
      if (!existing) {
        existing = getBundledXjDraft();
        await saveXjImportDraft(existing);
      }
      if (active) {
        setDraft(existing);
        setLoading(false);
  ...
  const fetchLive = async () => {
    ...
    try {
      const res = await fetch("/api/catalogue/fetch", {
        ...
        body: JSON.stringify({ includeDiagram: true, family: "xj" }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Fetch failed");
      await saveXjImportDraft(data.draft);
      setDraft(data.draft);
  ...
  const approveAllIllustrated = async () => {
    if (!draft) return;
    const next: XjSeriesImportDraft = {
      ...draft,
      parts: draft.parts.map((p) =>
  ...
  const approveImport = async () => {
    ...
    try {
      ...
      await saveXjImportDraft(draft);
      const history = await approveXjSeriesImport(draft);
      setMessage(
  ...
  if (loading && !draft) return <LoadingSpinner label="Loading XJ Series review…" />;

  return (
    <div className="space-y-5 pb-8">
      <PageHeader title="XJ Series Review" onBack={() => router.push(`${DAVEY_BASE}/settings`)} />
