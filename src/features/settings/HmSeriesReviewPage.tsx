...
import { getBundledHmDraft } from "@/lib/ricambio/parse-hm-series";
import {
  approveHmSeriesImport,
  getLatestHmDraft,
  saveHmImportDraft,
} from "@/services/hmSeriesImportService";
import { DAVEY_BASE } from "@/utils/brands";
import type { DraftPartRecord, HmSeriesImportDraft } from "@/types/catalogue-import";

export function HmSeriesReviewPage() {
  const router = useRouter();
  const [draft, setDraft] = useState<HmSeriesImportDraft | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      let existing = await getLatestHmDraft();
      if (!existing) {
        existing = getBundledHmDraft();
        await saveHmImportDraft(existing);
      }
      if (active) {
        setDraft(existing);
        setLoading(false);
  ...
  const fetchLive = async () => {
    ...
    try {
      const res = await fetch("/api/catalogue/fetch", {
        ...
        body: JSON.stringify({ includeDiagram: true, family: "hm" }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Fetch failed");
      await saveHmImportDraft(data.draft);
      setDraft(data.draft);
  ...
  const approveAllIllustrated = async () => {
    if (!draft) return;
    const next: HmSeriesImportDraft = {
      ...draft,
      parts: draft.parts.map((p) =>
  ...
  const approveImport = async () => {
    ...
    try {
      ...
      await saveHmImportDraft(draft);
      const history = await approveHmSeriesImport(draft);
      setMessage(
  ...
  if (loading && !draft) return <LoadingSpinner label="Loading HM Series review…" />;

  return (
    <div className="space-y-5 pb-8">
      <PageHeader title="HM Series Review" onBack={() => router.push(`${DAVEY_BASE}/settings`)} />
